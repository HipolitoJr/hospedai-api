...
urlpatterns = [
    ...
    url(r'^hoteis/(?P<pk>\d+)/hospedagens/$',
        views.HospedagemViewSet.as_view({'get': 'list'}), name='hospedagens'),
    url(r'^hoteis/(?P<pk>\d+)/hospedar/(?P<hospede_pk>\d+)/$',
        views.HospedagemPostViewSet.as_view({'post': 'create'}), name='hospedar'),
